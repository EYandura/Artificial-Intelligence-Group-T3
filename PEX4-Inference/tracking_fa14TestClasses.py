...
class DoubleInferenceAgent(bustersAgents.BustersAgent):
    ...
    def final(self, gameState):
        ...
        if not self.display.checkNullDisplay():
            time.sleep(3)
        # Used for debugging to write solution to a file
        # with open("output.solution", "a") as myfile:
        #     myfile.write("%d %s %s\n" % (self.numMoves, action, self.ghostBeliefs))
    ...
    def registerInitialState(self, gameState):
        ...
        self.firstMove = True
        # Used for debugging to write solution to a file
        # with open("output.solution", "w") as myfile:
        #     myfile.write("%d %s %s\n" % (self.numMoves, action, self.ghostBeliefs))
